...
def get_rank(joueur):
    ...
    # 4️⃣ Détecter les colonnes dynamiquement
    col_w = None
    col_l = None
    
    # Chercher colonne winner
    for col in df_base.columns:
        if 'winner' in col.lower():
            col_w = col
            break
    
    # Chercher colonne loser
    for col in df_base.columns:
        if 'loser' in col.lower():
            col_l = col
            break
    
    # 5️⃣ Si colonnes non trouvées, retourner par défaut
        ...
        if nom_a and len(nom_a) >= 3 and not st.session_state.get("ignorer_api"):
            ...
            if matchs_api:
                ...
                for match in matchs_api[:2]:
                    ...
                    with col_no:
                        if st.button("❌ Ignorer", key=f"ignore_a_{match['joueur_a']}_{match['joueur_b']}"):
                            st.session_state["ignorer_api"] = True
                st.markdown("---")
                ...
                for match in matchs_api_b[:2]:
                    ...
                    with col_ok2:
                        if st.button("Utiliser ce match", key=f"use_match_b_{match['joueur_a']}_{match['joueur_b']}"):
                            ...
                            st.session_state["ignorer_api"] = True
                    with col_no2:
                        if st.button("Ignorer", key=f"ignore_b_{match['joueur_a']}_{match['joueur_b']}"):
                            st.session_state["ignorer_api"] = True
                st.markdown("---")
